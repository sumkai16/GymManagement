...
// Handle AJAX actions (e.g., end_workout) BEFORE any other includes to keep JSON clean
...
<body>
    ...
    <script>
        ...
        // Live duration counter for in-progress workout (with seconds)
        (function(){
            if (!WORKOUT_ENDED) {
                const el = document.getElementById('duration_value');
                if (el) {
                    // Primary: use server-computed initial seconds to avoid timezone differences
                    let sec = parseInt(el.getAttribute('data-initial') || '0', 10);
                    const created = parseInt(el.getAttribute('data-created') || '0', 10);
                    // If initial is NaN, fall back to created timestamp computation
        ...
        function saveExercise(weId){
            const sets = document.getElementById(`sets_${weId}`).value;
            const reps = document.getElementById(`reps_${weId}`).value;
            const weight = document.getElementById(`weight_${weId}`).value;
            const fd = new FormData();
            fd.append('action','update_exercise');
        ...
        function addSet(weId){
            if (WORKOUT_ENDED) { alert('Workout has ended. Edits are disabled.'); return; }
            // next set number based on existing rows in container
